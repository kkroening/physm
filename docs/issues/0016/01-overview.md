...
definition's body holds at most one `Children` placeholder. There are no names in
the document except component names and the `id` on an anchor.
...
- **The subset grows deliberately.** Parameters, then expressions, then slots,
  then repetition. Each step is a widening of what the document can say, with a
  matching widening of the emitter and the reader.
